...
- **About page → showcase link** — the "Built in the open" section gained an optional second button ("See what others built") next to "View on GitHub", pointing at the locale-aware `/showcase` URL. Driven by a new `pages.about.openSource.showcaseButton` key in `en.json`/`nl.json`; the button only renders when the key exists, so removing the key (e.g. on a site without the showcase page) hides it with no template edit.
